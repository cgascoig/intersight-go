...
type AssetDeviceRegistration struct {
	...
	// The set of devices that are connected through this device and inherit this devices account ownership.
	//
	// Read Only: true
	LeafMembers []*AssetDeviceRegistrationRef `json:"LeafMembers"`
	...
	//
	// Enum: [ APIC DCNM UCSFI IMC IMCM4 IMCM5 HX HXTriton UCSD IntersightAppliance PureStorage VMware]
...
func (m *AssetDeviceRegistration) UnmarshalJSON(raw []byte) error {
	...
	m.LeafMembers = dataAO1.LeafMembers
...
func (m AssetDeviceRegistration) MarshalJSON() ([]byte, error) {
	...
	dataAO1.LeafMembers = m.LeafMembers
...
func (m *AssetDeviceRegistration) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateLeafMembers(formats); err != nil {
		res = append(res, err)
	}
...
func (m *AssetDeviceRegistration) validateLeafMembers(formats strfmt.Registry) error {

	if swag.IsZero(m.LeafMembers) { // not required
		return nil
	}

	for i := 0; i < len(m.LeafMembers); i++ {
		if swag.IsZero(m.LeafMembers[i]) { // not required
			continue
		}

		if m.LeafMembers[i] != nil {
			if err := m.LeafMembers[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("LeafMembers" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["","APIC","DCNM","UCSFI","IMC","IMCM4","IMCM5","HX","HXTriton","UCSD","IntersightAppliance","PureStorage","VMware"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
