...
type StoragePhysicalDisk struct {
	...
	// A collection of references to the [storage.PhysicalDiskExtension](mo://storage.PhysicalDiskExtension) Managed Object.
	//
	// When this managed object is deleted, the referenced [storage.PhysicalDiskExtension](mo://storage.PhysicalDiskExtension) MO unsets its reference to this deleted MO.
	//
	// Read Only: true
	PhysicalDiskExtension *StoragePhysicalDiskExtensionRef `json:"PhysicalDiskExtension,omitempty"`
...
func (m *StoragePhysicalDisk) UnmarshalJSON(raw []byte) error {
	...
	m.PhysicalDiskExtension = dataAO1.PhysicalDiskExtension
...
func (m StoragePhysicalDisk) MarshalJSON() ([]byte, error) {
	...
	dataAO1.PhysicalDiskExtension = m.PhysicalDiskExtension
...
func (m *StoragePhysicalDisk) Validate(formats strfmt.Registry) error {
	...
	if err := m.validatePhysicalDiskExtension(formats); err != nil {
		res = append(res, err)
	}
...
func (m *StoragePhysicalDisk) validatePhysicalDiskExtension(formats strfmt.Registry) error {

	if swag.IsZero(m.PhysicalDiskExtension) { // not required
		return nil
	}

	if m.PhysicalDiskExtension != nil {
		if err := m.PhysicalDiskExtension.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("PhysicalDiskExtension")
			}
			return err
		}
	}
