...
type VnicFcIf struct {
	...
	//
	// Enum: [fc-initiator fc-nvme-initiator fc-nvme-target fc-target]
...
var vnicFcIfTypeTypePropEnum []interface{}

func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["fc-initiator","fc-nvme-initiator","fc-nvme-target","fc-target"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
		vnicFcIfTypeTypePropEnum = append(vnicFcIfTypeTypePropEnum, v)
	}
}

// property enum
func (m *VnicFcIf) validateTypeEnum(path, location string, value string) error {
	if err := validate.Enum(path, location, value, vnicFcIfTypeTypePropEnum); err != nil {
		return err
	}
	return nil
}

func (m *VnicFcIf) validateType(formats strfmt.Registry) error {
